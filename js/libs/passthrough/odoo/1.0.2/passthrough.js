...
function main() {
    ...
    if (urlParams.get('testing_code') === null || urlParams.get('testing_code') === undefined || urlParams.get('testing_code') !== 'true') {
        ...
        for (let link of PageLinks) {
            ...
            const newHref = getNewLink(oldHref, urlParams);
...
document.addEventListener("DOMContentLoaded", function () {
    main();
    reapplyLinksWhenNeeded();
});

function reapplyLinksWhenNeeded() {
    const observer = new MutationObserver((mutationsList, observer) => {
        for (const mutation of mutationsList) {
            if (mutation.addedNodes.length > 0) {
                for (const node of mutation.addedNodes) {
                    if (node.nodeType === 1 && (node.tagName === 'A' || node.querySelector?.('a'))) {
                        console.log(`Nuevo enlace detectado en: <${anchor.tagName.toLowerCase()}> con texto: "${anchor.textContent.trim()}" y href: ${anchor.href}`);
                        main(); // Reejecuta main para actualizar nuevos enlaces
